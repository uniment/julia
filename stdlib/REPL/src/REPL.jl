...
function display(d::REPLDisplay, mime::MIME"text/plain", x)
    with_methodtable_hint(d.repl) do io
        repl = (d::REPLDisplay).repl   # #36454
        io = IOContext(io, :limit => true, :module => Main::Module)
        get(io, :color, false) && write(io, answer_color(repl))
        if isdefined(repl, :options) && isdefined(repl.options, :iocontext)
            # this can override the :limit property set initially
            io = foldl(IOContext, repl.options.iocontext, init=io)
        end
        show(io, mime, x)
        println(io)
    end
...
with_methodtable_hint(f, repl::AbstractREPL) = f(outstream(repl))
